...
export default function CollectionPage({ params }: CollectionPageProps) {
    const { id: categoryId, collectionId } = params;
    ...
    useEffect(() => {
        async function fetchCollection() {
            try {
                ...
                const response = await fetch(`/api/categories/${categoryId}/${collectionId}`);
            ...
            <div className="bg-brand-charcoal text-white py-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center gap-2 text-white/70 mb-6">
                        ...
                        <span>/</span>
                        <Link href={`/category/${categoryId}`} className="hover:text-white transition-colors">
                            {collection.category}
                        </Link>
                        <span>/</span>
                    ...
                    <p className="text-lg text-white/70">
                        {collection.imageCount} wallpapers available for download
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {collection.images.map((imageUrl, index) => {
                        // Extract parts from URL for the poster link
                        const urlParts = imageUrl.split('/');
                        const filename = urlParts[urlParts.length - 1];
                        const collectionName = urlParts[urlParts.length - 2];
                        const category = urlParts[urlParts.length - 3];
                        const posterUrl = `/poster/${encodeURIComponent(category)}/${encodeURIComponent(collectionName)}/${encodeURIComponent(filename)}`;
                ...
                {collection.images.length > 0 && (
                    <div className="text-center mt-12">
                        <p className="text-gray-500">
                            Showing all {collection.imageCount} wallpapers • Click any image to view and download
                        </p>
                    </div>
                )}
            </div>
